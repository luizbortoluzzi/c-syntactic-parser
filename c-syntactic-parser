...
void add_symbol(const char *name, int token)
{
    ...
    if (token == TK_Close_Curly_Bracket)
    {
        ...
        if (token == TK_Id)
        {
            ...
            if (token != TK_Semicolon)
            {
                printf("Line: %d Column: %d - Syntax error: Expecting ;\n", line, column);
                fprintf(output, "Syntax error: Expecting ;\n");
                return; // You may add error recovery if you want
            }
            else
            {
...
{
    // Você já verificou o token '[' antes de chamar esta função
    get_token(); // Consumir '['
    ...
    if (token != TK_Int_Const)
    {
        ...
        return; // Você pode adicionar um mecanismo de recuperação de erro se quiser
    }

    get_token(); // Consumir o tamanho
    ...
    // Verificando se temos mais dimensões para o array
    ...
    // Após a declaração de array, esperamos um ponto e vírgula
    if (token != TK_Semicolon)
    {
        printf("Line: %d Column: %d - Syntax error: Expecting ';'\n", line, column);
        fprintf(output, "Syntax error: Expecting ';'\n");
...
void declaration()
{
    ...
    if (token == TK_Semicolon)
    {
        ...
        get_token(); // Pula o ponto e vírgula e vai para a próxima declaração
    }
    else if (token == TK_Open_Curly_Bracket)
    {
        ...
        // Chame uma função para lidar com a declaração de struct
        struct_declaration();
    }
...
int main()
{
    ...
    for (int i = 0; i < symbol_count; i++)
    {
        printf("%s - Token: %d\n", symbol_table[i].name, symbol_table[i].token);
    }
